...
from detector.common import load_pickle, reconstruct_incoming_task
...
def create_manifest(
    ...
    counts = {
        "train": int(train_per_class),
        "validation": int(validation_per_class),
        "test": int(test_per_class),
    }
    ...
    for class_id in classes:
        ...
        if len(class_indices) <= selected_count:
            raise ValueError(
                "Class {} has {} samples, but {} selected samples and a non-empty reserve are required.".format(
                    class_id, len(class_indices), selected_count
                )
            )
        ...
        assignments = {
            ...
            "validation": class_indices[train_end:validation_end],
            "test": class_indices[validation_end:test_end],
            "reserve": class_indices[test_end:],
        }
...
def validate_manifest_table(manifest, targets):
    ...
    if manifest[list(expected_columns)].isna().any().any():
        raise RuntimeError(
            "Manifest identity, label and split fields must not be missing."
        )
    for column in ("original_index", "class_id"):
        values = manifest[column].to_numpy(dtype=np.float64)
        if (
            not np.isfinite(values).all()
            or not np.equal(values, np.floor(values)).all()
        ):
            raise RuntimeError(
                "Manifest {} must contain finite integers.".format(column)
            )

    if len(manifest) != len(targets):
        raise RuntimeError(
            "Manifest has {} rows; task 9 has {} samples.".format(
...
def validate_manifest(
    manifest, targets, train_per_class, validation_per_class, test_per_class
):
    """Also verify the requested per-class counts in the generated manifest."""
    ...
    for class_id in sorted(np.unique(targets).tolist()):
        ...
        for split_name, expected_count in expected_selected.items():
            ...
            if observed != expected_count:
                raise RuntimeError(
                    "class={}, split={}: expected {}, found {}.".format(
                        class_id, split_name, expected_count, observed
                    )
                )
        ...
        if observed_reserve != expected_reserve:
            raise RuntimeError(
                "class={}, reserve: expected {}, found {}.".format(
                    class_id, expected_reserve, observed_reserve
                )
            )
...
def main(args):
    ...
    if len(incoming_dataset) != 5000:
        raise RuntimeError(
            "Expected 5,000 task-9 training images, found {}.".format(
                len(incoming_dataset)
            )
        )
    ...
    if unique.tolist() != list(range(10)):
        raise RuntimeError(
            "Expected task-local labels 0..9, found {}.".format(unique.tolist())
        )

    if not np.all(counts == 500):
        raise RuntimeError(
            "Expected 500 images per class, found {}.".format(counts.tolist())
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Create the pre-attack task-9 manifest for the clean supervised detector baseline."
        )
    )
    parser.add_argument(
        "--checkpoint",
        required=True,
        help=(
            "Victim checkpoint trained on tasks 0-8, with checkpoint['task_num'] == 9."
        ),
    )
